...
public class RangeBar extends View {
    ...
    public static final int POINT_TO_TOP = 0;
    public static final int POINT_TO_LEFT = 1;
    public static final int POINT_TO_RIGHT =2;
    public static final int POINT_TO_BOTTOM =3;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        bundle.putInt("BAR_POINT", mBarPoint);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof Bundle) {
            ...
            mBarPoint = bundle.getInt("BAR_POINT");
        ...
        if (measureWidthMode == MeasureSpec.AT_MOST) {
            switch (mBarPoint)
            {
                ...
                case POINT_TO_RIGHT:
                    ...
                    width = (int) (mBarPaddingBottom+mCircleSize+mPinPadding+2*mExpandedPinRadius+TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40,mDisplayMetrices));
                    break;
                default:
                    width = measureWidth;
                    break;
        ...
        if (measureHeightMode == MeasureSpec.AT_MOST) {
            switch (mBarPoint)
            {
                ...
                case POINT_TO_RIGHT:
                    ...
                    break;
                default:
                    height = (int) (mBarPaddingBottom+mCircleSize+mPinPadding+2*mExpandedPinRadius+TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40,mDisplayMetrices));
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        float fixCo = 0;
        float barLength = 0;
        float marginLeft = Math.max(mExpandedPinRadius, mCircleSize);
        switch (mBarPoint)
        {
            case POINT_TO_LEFT:
                xPos = 2*mExpandedPinRadius+mCircleSize+mBarPaddingBottom+mPinPadding;
                fixCo = xPos;
                barLength = h-(2 * marginLeft);
                break;
            case POINT_TO_RIGHT:
                xPos = w - mBarPaddingBottom-mCircleSize-mPinPadding-2*mExpandedPinRadius;
                fixCo = xPos;
                barLength = h-(2 * marginLeft);
                break;
            case POINT_TO_TOP:
                yPos = 2*mExpandedPinRadius+mCircleSize+mBarPaddingBottom+mPinPadding;
                fixCo = yPos;
                barLength = w - (2 * marginLeft);
                break;
            default:
                yPos = h - mBarPaddingBottom-mCircleSize-mPinPadding-2*mExpandedPinRadius;
                fixCo = yPos;
                barLength = w - (2 * marginLeft);
                break;
        }
        ...
        if (mIsRangeBar) {
            ...
            mLeftThumb.init(ctx, fixCo, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
                    mCircleColor, mCircleBoundaryColor, mCircleBoundarySize, mMinPinFont, mMaxPinFont, mArePinsTemporary,mBarPoint);
        }
        mRightThumb = new PinView(ctx);
        mRightThumb.setFormatter(mFormatter);
        mRightThumb.init(ctx, fixCo, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
                mCircleColor, mCircleBoundaryColor, mCircleBoundarySize, mMinPinFont, mMaxPinFont, mArePinsTemporary,mBarPoint);

        // Create the underlying bar.
        mBar = new Bar(ctx, marginLeft, fixCo, barLength, mTickCount, mTickHeight, mTickColor,
                mBarWeight, mBarColor,mBarPoint);
        ...
        switch (mBarPoint)
        {
            ...
            case POINT_TO_RIGHT:
                if (mIsRangeBar) {
                    mLeftThumb.setY(marginLeft + (mLeftIndex / (float) (mTickCount - 1)) * barLength);
                    mLeftThumb.setYValue(getPinValue(mLeftIndex));
                }
                mRightThumb.setY(marginLeft + (mRightIndex / (float) (mTickCount - 1)) * barLength);
                mRightThumb.setYValue(getPinValue(mRightIndex));
                break;
            default:
                if (mIsRangeBar) {
        ...
        mConnectingLine = new ConnectingLine(ctx, fixCo, mConnectingLineWeight,
                mConnectingLineColor,mBarPoint);
    ...
    public float getTickPixel()
    {
        return mRightThumb.getY()-mLeftThumb.getY();
    }

    public float getTickDistance()
    {
        return mBar.getTickDistance();
    }
    ...
    private void rangeBarInit(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mBarPoint = ta.getInt(R.styleable.RangeBar_mrb_barPoint,POINT_TO_TOP);
        } finally {
            ta.recycle();
        }
    }
    ...
    private void createPins() {
        ...
        if (mIsRangeBar) {
            ...
            mLeftThumb.init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize, mCircleColor, mCircleBoundaryColor, mCircleBoundarySize,
                    mMinPinFont, mMaxPinFont, false,mBarPoint);
        }
        mRightThumb = new PinView(ctx);
        mRightThumb
    ...
    private float getYPos() {
        ...
        switch (mBarPoint) {
            case POINT_TO_LEFT:
                return 2*mExpandedPinRadius+mCircleSize+mBarPaddingBottom+mPinPadding;
            case POINT_TO_RIGHT:
                return getWidth()- mBarPaddingBottom-mCircleSize-mPinPadding-2*mExpandedPinRadius;
            case POINT_TO_TOP:
                return 2*mExpandedPinRadius+mCircleSize+mBarPaddingBottom+mPinPadding;
            default: {
                return getHeight() - mBarPaddingBottom - mCircleSize - mPinPadding - 2 * mExpandedPinRadius;
            }
        }
    }
    ...
    private float getBarLength() {
        switch (mBarPoint)
        {
            case POINT_TO_LEFT:
            case POINT_TO_RIGHT:
                return (getHeight() - 2 * getMarginLeft());
            default:
    ...
    private void onActionUp(float x, float y) {
        if (mIsRangeBar && mLeftThumb.isPressed()) {
            ...
            switch (mBarPoint)
            {
                case POINT_TO_LEFT:
                case POINT_TO_RIGHT: {
                    float leftThumbXDistance = mIsRangeBar ? Math.abs(mLeftThumb.getY() - y) : 0;
                    float rightThumbXDistance = Math.abs(mRightThumb.getY() - y);
    ...
    private void onActionMove(float x,float y) {
        ...
        if (mIsRangeBar && mLeftThumb.isPressed()) {
            movePin(mLeftThumb, x,y);
        } else if (mRightThumb.isPressed()) {
            movePin(mRightThumb, x,y);
        }
        ...
        switch (mBarPoint)
        {
            ...
            case POINT_TO_RIGHT:
                ...
                if (mIsRangeBar && mLeftThumb.getY() > mRightThumb.getY()) {
        ...
        switch (mBarPoint)
        {
            ...
            case POINT_TO_RIGHT:
                if (x <= componentLeft) {
                    newLeftIndex = 0;
                    movePin(mLeftThumb,0,mBar.getLeftX());
                } else if (x >= componentRight) {
                    newRightIndex = getTickCount() - 1;
                    movePin(mRightThumb, 0,mBar.getRightX());
                }
            ...
            default:
                if (x <= componentLeft) {
                    newLeftIndex = 0;
                    movePin(mLeftThumb, mBar.getLeftX(),0);
                } else if (x >= componentRight) {
                    newRightIndex = getTickCount() - 1;
                    movePin(mRightThumb, mBar.getRightX(),0);
                }
    ...
    private void releasePin(final PinView thumb) {
        ...
        switch (mBarPoint)
        {
            ...
            case POINT_TO_RIGHT:
                ...
                thumb.setYValue(getPinValue(tickIndex));
                break;
            default:
     ...
     */
    private void movePin(PinView thumb, float x,float y) {
        ...
        switch (mBarPoint)
        {
            case POINT_TO_LEFT:
            case POINT_TO_RIGHT:
                if (y < mBar.getLeftX() || y > mBar.getRightX()) {
                    // Do nothing.
                } else if (thumb != null) {
                    thumb.setY(y);
                    invalidate();
