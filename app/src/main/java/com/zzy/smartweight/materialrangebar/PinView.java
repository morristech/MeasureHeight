...
class PinView extends View {
     ...
     *
     * @param ctx                 Context
     * @param fixPoint            The x or y coordinate to raw the pin (i.e. the bar location)
     ...
     */
    public void init(Context ctx, float fixPoint, float pinRadiusDP, int pinColor, int textColor,
                     float circleRadius, int circleColor, int circleBoundaryColor, float circleBoundarySize, float minFont, float maxFont, boolean pinsAreTemporary,int barPoint) {

        mRes = ctx.getResources();
        mBarPoint = barPoint;
        switch (mBarPoint)
        {
            case RangeBar.POINT_TO_TOP:
                mPin = ContextCompat.getDrawable(ctx, R.drawable.rotate);
                break;
            default:
                mPin = ContextCompat.getDrawable(ctx, R.drawable.rotate_bottom);
                break;
        }
        ...
        mCirclePaint.setStrokeWidth(circleBoundarySize);
    ...
    /**
     * Set the y value of the pin
     *
     * @param y set x value of the pin
     */
    @Override
     ...
     */
    public void setYValue(String x) {
        mValue = x;
    }
    ...
    public void draw(Canvas canvas) {
        ...
        {
            case RangeBar.POINT_TO_LEFT:
                canvas.drawLine(mX,mY,canvas.getWidth()-10,mY,mCirclePaint);
                break;
            case RangeBar.POINT_TO_RIGHT:
                canvas.drawLine(mX,mY,10,mY,mCirclePaint);
                break;
            case RangeBar.POINT_TO_TOP:
                canvas.drawLine(mX,mY,mX,canvas.getHeight()-10,mCirclePaint);
                break;
            default:
                canvas.drawLine(mX,mY,mX,10,mCirclePaint);
                break;
        }
        ...
        if (mPinRadiusPx > 0 && (mHasBeenPressed || !mPinsAreTemporary)) {
            int pinLeft,pinRight,pinTop,pinBottom;
            float textX,textY;
            switch (mBarPoint)
            {
                case RangeBar.POINT_TO_LEFT:
                    pinLeft =(int)mX - (mPinRadiusPx * 2) - (int) mPinPadding;
                    pinRight =(int) mX - (int) mPinPadding;
                    pinTop = (int) mY - mPinRadiusPx;
                    pinBottom = (int) mY + mPinRadiusPx;
                    textX = mX - mPinRadiusPx - mPinPadding + mTextYPadding;
                    textY = mY;
                    break;
                case RangeBar.POINT_TO_RIGHT:
                    pinLeft =(int) mX + (int) mPinPadding;;
                    pinRight = pinLeft+2*mPinRadiusPx;;
                    pinTop = (int) mY - mPinRadiusPx;
                    pinBottom = (int) mY + mPinRadiusPx;
                    textX = pinLeft+mPinRadiusPx+mTextYPadding;
                    textY = mY;
                    break;
                case RangeBar.POINT_TO_TOP:
                    pinLeft =(int) mX - mPinRadiusPx;
                    pinRight =(int) mX + mPinRadiusPx;
                    pinTop = (int) mY - (mPinRadiusPx * 2) - (int) mPinPadding;
                    pinBottom = (int) mY - (int) mPinPadding;
                    textX = mX;
                    textY = mY - mPinRadiusPx - mPinPadding + mTextYPadding;
                    break;
                default:
                    pinLeft =(int) mX - mPinRadiusPx;
                    pinRight =(int) mX + mPinRadiusPx;
                    pinTop = (int) mY + (int) mPinPadding;
                    pinBottom = pinTop+2*mPinRadiusPx;
                    textX = mX;
                    textY = pinBottom-mPinRadiusPx+mTextYPadding;
                    break;

            }
            mBounds.set(pinLeft, pinTop, pinRight, pinBottom);
