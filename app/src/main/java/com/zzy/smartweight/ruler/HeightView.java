package com.zzy.smartweight.ruler;
        ...
        switch (MeasureSpec.getMode(heightMeasureSpec)) {
                ...
                switch (mOrientation) {
                    ...
                    default: {
                        if(screenH!=0)
                        {
                            ...
                            Common.LogEx("measureHeight viewPhysicsH:"+viewPhysicsH);
                        }
                        return (int)(mLines * space);
                    }
                }
            ...
            default:
                if(screenH!=0)
                {
                    space = screenH/physicsH;
                    double viewPhysicsH = physicsH*getMeasuredHeight()/screenH;
                    Common.LogEx("measureHeight viewPhysicsH 1:"+viewPhysicsH);
                }
        ...
        if (mOverScroller.computeScrollOffset()) {
            ...
            switch (mOrientation) {
                case VERTICAL:
                    ...
                    Common.LogEx("scrollTo 1");
                    break;
                case HORIZONTAL:
                    scrollTo(mOverScroller.getCurrX(), 0);
                    Common.LogEx("scrollTo 2");
                    break;
            }
            ViewCompat.postInvalidateOnAnimation(this);
        } else {
...
//        //绘制当前选中的线条
    ...
    public void setPhysicSize(double physicsW,double physicsH,int screenW,int screenH)
    {
        ...
        Common.LogEx("setPhysicSize viewPhysicsH:"+viewPhysicsH);
    }
    ...
    public void setCurrentLineIndex(int currentLineIndex) {
        ...
        switch (mOrientation) {
            case HORIZONTAL:
                ...
                Common.LogEx("scrollTo 3");
                break;
            case VERTICAL:
                int startY = ((getHeight() >> 1) - getPaddingBottom());
                scrollTo(getScrollX(), -distance + startY);
                Common.LogEx("scrollTo 4");
                break;
        }
        adjustMarker(true);
        postInvalidate();
